...
| `/menu` | Full broadsheet menu with sticky filter chips (All · Veggie · Meaty · Spicy · Sweet) and `Menu` JSON-LD. |
| `/cocktails` | Split view of cocktails + flights with the bubble overlaid on a real cocktail photo. |
| `/the-cause` | Charity pie spotlight + month-by-month archive. |
| `/visit` | Hours, address, parking, transit, accessibility, full photo gallery. |
| `/private-events` | Three event kits (daytime / evening / bar-only) + mailto inquiry. |
